...
- **The workspace** lives on a volume named `shared` mounted at `/__w`. Declare one yourself (for example an ephemeral PVC, as in the `large` class of [`examples/values/classes.yaml`](examples/values/classes.yaml)); otherwise a 10Gi `emptyDir` is used. The runner's staging area (`/__w/_act`), `RUNNER_TEMP` (`/__w/_temp`) and `RUNNER_TOOL_CACHE` (`/__w/_tool`) live there too.
- **The tool cache** follows the job image. If `main`'s environment (image `ENV` or podspec `env`) sets `RUNNER_TOOL_CACHE`, or else `AGENT_TOOLSDIRECTORY`, to an absolute directory the step user can write, `/__w/_tool` becomes a symlink to it, so tools baked into the image (for example the Node in `catthehacker/ubuntu`) are found by `actions/setup-*` instead of downloaded every job. Versions a job adds then land in the container's writable layer rather than on the `shared` volume. Otherwise, or when the podspec already mounts something at `/__w/_tool`, it is a plain per-job directory on the volume. To pin it, set `RUNNER_TOOL_CACHE` in `main`'s podspec `env`.
