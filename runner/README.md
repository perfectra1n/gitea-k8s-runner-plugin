...
| Patch | Adds |
|---|---|
| 0001 `act/plugin` | A client for the `plugin.v1alpha` gRPC protocol (connect-go in gRPC mode, which upstream already depends on) and `plugin.Environment`, a `container.ExecutionsEnvironment` backed by it. Includes the vendored protocol code and `plugintest`, an in-process fake plugin. |
...
| 0004 `run` | Wiring: labels are parsed with the configured plugins, registered and declared by name, and jobs are routed to the plugin of the first matching `runs-on` label. |
...
`runner:apply` refuses to discard uncommitted work or commits that are not in `patches/` yet (use `FORCE=1` to discard them). After changing `proto/`, run `mise run runner:gen` to regenerate the vendored protocol inside the clone and commit it there; `mise run runner:gen-check` fails when the two differ.
