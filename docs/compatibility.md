...
- **The vendored protocol in the runner patches** is generated from the same `.proto` (`buf.gen.runner.yaml`); `mise run runner:gen-check` fails if the two drift.

## Semantics this implementation follows

- Per environment the runner issues one RPC at a time; a cancelled stream (job cancelled, step timed out) may be followed immediately by `Remove`. The plugin serialises RPCs per environment and kills the running command when an `Exec` stream is cancelled.
...
- `Remove` is idempotent, including for environment ids the plugin process never saw (it deletes by name, which covers a plugin restart).
